...
try:
    ...
    on id
    '''
    # incremental_data_df = df_upper.join(existing_hive_data, on='id', how='left-anti')
    ...
    incremental_data_df_anti.show()

    # counting the number of the new records added to postgres tables
    new_records = incremental_data_df_anti.count()
    ...
    if incremental_data_df_anti.count() > 0:
        # Append new rows to Hive table
        incremental_data_df_anti.write.mode("append").insertInto("{}.{}".format(hive_database_name, hive_table_name))
        #incremental_data_df.write.mode('overwrite').saveAsTable("{}.{}".format(hive_database_name, hive_table_name))
        print("Appended {} new records to Hive table.".format(incremental_data_df_anti.count()))
    else:
